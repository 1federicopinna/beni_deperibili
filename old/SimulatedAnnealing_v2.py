...
from Utilities import discr_cont_distrib, Theoretical_SsI_Values, Costs, Policy, CR, debug_report
...
class SA:
    ...
    def optimize_fixed_I_2(self, i, *,  # di seguito parmetri opzionali che sono anche attributi dell'oggetto SA
                           T_start: Optional[float] = None, T_end: Optional[float] = None,
                           T_cooling_rate: float = 0.95,  # fattore riduzione della temperatura - cooling geometrico
                           n_days: int = 100,  # numero di giorni di ogni simulazione
                           init_acc_prob: float = 0.3,  # probabilità di accettazione iniziale
        ...
        # best_obj_sim a 0
        best_sol, best_obj_sim = (values['S'], values['s'],
                                  i), 0  # la tupla è la chiave che verrà salvata nel dizionario per valutare l'andamento della fitness

        # valuto la best obj iniziale con il modello di regressione
        best_obj_reg = model_predict(*best_sol)
        self.improvements[i] = {0: best_obj_reg}
        ...
        while T >= T_end:
            ...
            while n_sol_run[1] <= n_run_era:  # numero fisso di repliche ad ogni temperatura
                ...
                obj_old = model_predict(*current_sol)
                obj_new = model_predict(S, s, i)

                delta_mean = obj_new - obj_old
                accepted = accept_solution(delta_mean, T)  # Usa la funzione corretta

                if accepted:
                    current_sol = (S, s, i)
                    if obj_new > best_obj_reg or obj_new >= best_obj_reg * (1 - 0.01):

                        print(
                            f"Soluzione vicina al best: current sol: {current_sol} obj_new={obj_new:.1f} vs "
                            f"best_obj*(1-0.01)={best_obj_reg * (1 - 0.01):.1f}")

                        # in questo caso bisogna verificare anche obj_new e best_obj simulati con fidelity alta
                        best_obj_sim = self.evaluate_solution(*best_sol, f=3, n_days=n_days, n_sol_run=None)
                        obj_new_sim = self.evaluate_solution(*current_sol, f=3, n_days=n_days, n_sol_run=None)

                        # aggiorno il modello
                        aggiornamento_X(*best_sol, best_obj_sim)
                        aggiornamento_X(*current_sol, obj_new_sim)
                        n_aggiornamenti_X += 2
                        mean_best_obj_sim = stats.mean(best_obj_sim)
                        mean_obj_new_sim = stats.mean(obj_new_sim)

                        if mean_obj_new_sim > mean_best_obj_sim:
                            print(
                                f"TROVATO NUOVO BEST: la soluzione {current_sol} con obj_new: {obj_new} ha "
                                f"dato esito mean_obj_new_sim: {mean_obj_new_sim} maggiore della best_sol: {best_sol},"
                                f"con delta_best_obj_mean {mean_best_obj_sim}. la best_obj_reg è {best_obj_reg}")
                            best_sol = (S, s, i)
                            best_obj_reg = obj_new  # fo ottima dal regressore
                            print(f"nuovo best_obj_reg: {best_obj_reg}")
                            best_obj_sim = mean_obj_new_sim  # fo ottima dalla simulazione
                            self.fo_storico.append(round(best_obj_reg, 2))

                            no_improvement = 0  # quando trovo un a soluzione che accetto
                ...
                self.storico_iterazioni.append((
                    n_sol_run[0],
                    round(T, 3),  # temperatura
                    round(obj_old, 2),  # obj corrente
                    round(obj_new, 2),  # obj nuovo
            ...
            obj_curr_sim = self.evaluate_solution(*current_sol, f=3, n_days=n_days, n_sol_run=n_sol_run)
            aggiornamento_X(*current_sol, obj_curr_sim)
            n_aggiornamenti_X += 1

            if n_aggiornamenti_X >= 6:
                aggiornamento_modello()
                print("Modello aggiornato")
                n_aggiornamenti_X = 0
            ...
            obj_curr_sim_mean = stats.mean(obj_curr_sim) if isinstance(obj_curr_sim, list) else obj_curr_sim
            print(
                f"Fine Epoca - T={T:.1f} impostato: {n_no_imp} - run senza imp: {no_improvement} | current_sol={current_sol} obj_curr_sim_mean={obj_curr_sim_mean:.1f} |"
                f"best_sol={best_sol} best_obj={best_obj_reg:.1f}")
