...
def objective_function(target_fr:float, daily_penalty:float) -> Callable:
    ...
    def inner(results:dict[str, list[float]]) -> list[float]:
        ...
        return [round(rev, 2) - round(nd*daily_penalty*max(0, target_fr - fr), 2) for rev, fr in zip(results['revenues'], results['fill_rate'])]
    return inner
...
def neighbor_function(Sm:int = 1, SM:int = mt.inf, sm:int = 1, sM:int = mt.inf, # valore minimo e massimo di S, valore minimo s e massimo di s 
    ...
    def inner(S:int, s:int, i:int = 1, dT:float = 0):
        ...
        if rn.random() <= sum(prob[:-1]): qS, qs = qs, qS # tutto su s, scambiamo i valori 
        else:
...
def check_fidelity(T:float, Th:float, deltas:list[float], fidelity:int = 1) -> bool:
    ...
    if T >= Th: # al massimo si va a fidelity 2, se siamo a 1 ci fermiamo se almeno il 75% delle osservazioni è vincente
        if (n_wins/nd >= 0.75 and d_mean >= 0) or (n_loss/nd >= 0.75 and d_mean <= 0) or fidelity > 1 : return True  
        else: return False
    
    # tempereatura fredda
    if T < Th:
        if fidelity == 1 and (n_wins == nd or n_loss == nd):  return True # in questo caso vogliamo che siano tutte vincenti
        if fidelity == 2: # qui vogliamo almeno 75% vincenti o errore basso
            st_error = stats.stdev(deltas)/mt.sqrt(nd)
            z = abs(d_mean)/st_error 
            if (n_wins/nd >= 0.75 and d_mean >= 0) or (n_loss/nd >= 0.75 and d_mean <= 0) or z >= 3: return True # ci accontentiamo del 75%
            else: return False
...
class SA:
    ...
    # DATA & DISTRIBUTION
    lead_time = triang(c = 0.8, loc = 0.5, scale = 3) # c posizione della moda 0.5 centrale, loc estremo inferiore, scale ampiezza intervallo
    shelf_life = discr_cont_distrib(triang(c = 0.8, loc = 5, scale = 7), [i for i in range(1, 100)]) # shelf life discretizzata
    ...
    # COSTS & REVENUES
    Cr = CR(sale_price = 10, cost  = Costs(pc = 5, oc = 500, dc = 0.5, u = 0, h = 0.1)) # non consideriamo costo mancata vendita, dato che includiamo il fill rate come vincolo
    ...
    def __init__(self, f_obj:Callable, f_ngh: Callable, f_fid: Callable, # le tre funzioni 
                 fidelity:dict[int, int] # la fidelity: livello di fidelity e corrispondete numero di seed  - i seed devono essere incrementali, per ridurre varianza
                 ):
        """ funzioni di input 
        ...
        self.fid = {fid:n_seeds for fid, n_seeds in sorted(fidelity.items())} #F. ordina i livelli di fidelity passati in inp
        self.sds ={fid:tuple(range(1,5*fidelity[fid], 5)) for fid in fidelity.keys()} # i seed di ogni livello di fidelity, a step di 5
    ...
    def __getitem__(self, Ssi:tuple[int]) -> tuple[dict[str, list[float]],int]|tuple[None, None]: # restituisce i valori della cache
        ...
        for f in self._fid: # la fidelity massima a scendere
            try: return self.cache[(S,s,i,f)], f # restituisce i valori letti nella cache
            except: continue
        return None, None
    ...
    def evaluate_solution(self, S:int, s:int, i:int, f:int,*, n_days:int = 100, n_sol_run:Optional[list[int, int]] = None) -> list[float]:
        ...
        if f_existing is not None: # trovato ma a fidelity minore, alcune simulazioni vanno rilanciate 
            if f_existing >=  f: return self.f_obj(old_results) # trovato ad una fidelity maggiore o uguale (se maggiore, avremo più valori di quanti richiesti, ma non importa)
            ns = len(old_results['revenues']) # consideriamo le revenue, ma un output vale l'altro, ci interessa solo capire quanti seed nuovi dovanno essere valutati
            seeds = self.sds[f][ns:] # seed rimanenti da usare 
        else: seeds = self.sds[f]
        if n_sol_run: 
    ...
    def simulate(self, S:int, s:int, i:int, seeds:Iterable[int] = tuple(i for i in range(1, 6)), n_days:int = 100):
        ...
        for seed in seeds:
            ...
            V = Vendor(env, LT_distrib = SA.lead_time, SL_distrib = SA.shelf_life, product_kind = "Pr", min_lt = SA.vendor_min_lt)
            ...
            env.process(B.gen_daily_demand(demand = SA.daily_demand, 
                                           pr_long_sl = SA.pr_long_shelf_life, split_factor = SA.n_demand_split, print_out = False))
            env.process(B.periodic_order())
            env.run(until = n_days)
            for key, val in zip(results.keys(), (B.tot_revenue, B.pr_stock_out, B.fill_rt, B.average_oh, B.I_triggered_orders, B.lost_sales)):
    ...
    def Gen_T(self, S, s, i, init_acc_prob:float = 0.3, end_acc_prob:float = 0.01,
        ...
        for rp in range(1, nsol+1):
            ...
            if not (S, s) in solutions:
                solutions.add((S, s))
                obj = self.f_obj(self.simulate(S, s, i, seeds = (seed,), n_days = n_days))[0] # [0] serve per prendere la prima e unica soluzione generata 
                bisect.insort(objectives, obj) # teniamo la lista ordinata in senso crescente
            S, s = self.f_ngh(S, s) # con valore di default dT = 0
        delta_obj = [objectives[-1] - of for of in objectives[:-1]] # gli errori riseptto alla miglior soluzione trovata
    ...
    def optimize_fixed_I(self, i, *, # di seguito parmetri opzionali che sono anche attributi dell'oggetto SA
        ...
        _, values = Theoretical_SsI_Values(SA.daily_demand,
                                           ...
                                           I = i, safety_level = 1 - SA.stock_out_prob) # i valori teorici S, s da cui si parte
        
        best_sol, best_obj = (values['S'], values['s'], i), 0  # la tupla è la chiave che verrà salvata nel dizionario per valutare l'andamento della fitness
        ...
        while T >= T_end:
            ...
            print(to_print)
            n_sol_run[1] = 0  # riportimo a zero solo il secondo valore (run) che serve come criterio di stop, l'altro deve incrementare sempre
            ...
            while n_sol_run[1] <= n_run_era: # numero fisso di repliche ad ogni temperatura 
                ...
                while True: # partiamo da fidelity 1 e poi via via cresciamo sino a quando serve
                    ...
                    delta_obj = self.compare_solutions(obj_new, obj_old) # confrontiamo e otteniamo il vettore dei delta 
                ...
                no_improvement += 1 # aumentiamo il numero di non miglioramenti consecutivi, se poi miglioriamo lo riportiamo a zero
                if  accept_solution(delta_mean, T): # probabilità di accettazione, con delta_obj è negativo!!! 
                    current_sol = (S, s, i)
                    # Parte 2.3 Se accettiamo vediamo se la soluzione sostituisce l'attuale migliore 
                    """ Oss. per evitare di scambiare un falso ottimo qui bisognerebbe rivalutare la funzione obiettivo 
                        del current best e del new best con una very high fidelity 
                        ... per ora non stiamo a farlo """
                    mean_obj = stats.mean(obj_new)
                    if mean_obj > best_obj:
                        best_sol, best_obj = (S, s, i), mean_obj # aggiorniamo il best
                        no_improvement = 0

                self.improvements[i][(n_sol_run[1], best_sol)] = round(best_obj, 2) # registriamo la soluzione nel dizionario improvement, può essere la stessa di prima                if no_improvement > n_no_imp:
                if no_improvement > n_no_imp: # condizionie di chiusura
                    print('OPTIMIZATION ENDS')
                    if not tot_time: return best_sol, best_obj
                    else: return best_sol, best_obj, round(time.perf_counter() -  t_start, 2) 
    ...
    def __call__ (self, i_values:Iterable[int], *, # di seguito parmetri opzionali che sono anche attributi dell'oggetto SA
                             T_start:Optional[float] = None, T_end:Optional[float] = None, 
                             T_cooling_rate: float = 0.95, # fattore riduzione della temperatura - cooling geometrico
                             n_days:int = 100,
                             init_acc_prob:float = 0.3, # probabilità di accettazione iniziale
